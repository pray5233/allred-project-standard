...
当前预发布版本：`0.8.0-rc5`；最新稳定版仍为 `0.7.1`。安装目录中的 `VERSION` 和安装回执用于确认实验机实际加载的版本。
...
稳定版本应使用与 `VERSION` 一致的 Git tag；当前稳定 tag 为 `v0.7.1`，GitHub 预发布 tag 为 `v0.8.0-rc5`。`0.8.0-rc5` 仍是候选版本，不替代稳定版；升级稳定版需要单独验收和发布。
