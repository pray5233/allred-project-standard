...
当前本地候选版本：`0.8.0-rc11`。远端是否已发布以 GitHub Releases 和安装目录中的 `VERSION` 为准；安装回执用于确认实验机实际加载的提交与哈希。
...
稳定版本应使用与 `VERSION` 一致的 Git tag；当前稳定 tag 为 `v0.7.1`，本次 GitHub 预发布 tag 为 `v0.8.0-rc11`。`rc11` 是实验机候选，不替代稳定版，完成异机实测后再决定是否晋升稳定版本。
