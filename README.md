...
当前本地候选版本：`0.8.0-rc13`。远端是否已发布以 GitHub Releases 和安装目录中的 `VERSION` 为准；安装回执用于确认实验机实际加载的提交与哈希。
...
`0.8.0-rc13` 修复自动化动态评测发现的阶段边界缺陷：用户说资料齐全但没有给出可读取位置时，Codex 先在当前工作区做一次有限搜索；没有路径、附件或可信命中就保持 INTAKE，只询问准确位置，不再通过读取 EVIDENCE 规则假装进入证据阶段。维护端同时将FailFast后未执行案例标记为 `NotRun`，并修复GitHub Actions报告摘要。
...
稳定版本应使用与 `VERSION` 一致的 Git tag；当前稳定 tag 为 `v0.7.1`，最新已发布 GitHub 预发布 tag 为 `v0.8.0-rc12`。本地 `rc13` 正在进行自托管动态评测，完成后再决定是否发布新候选。
