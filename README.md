...
当前本地候选版本：`0.8.0-rc14`。GitHub 最新预发布版本为 `0.8.0-rc13`，稳定版本仍为 `0.7.1`；安装目录中的 `VERSION` 和安装回执用于确认实验机实际加载的版本、提交与哈希。
...
它适用于 App、脚本、自动化、数据整理、培训、制度、知识库、标书、合同、检验记录、设备相关分析和长期任务，不只适合新建软件。
...
`0.8.0-rc7` 针对 rc6 实测中出现的语义反转、校验链状态不可靠和写入边界不透明问题，增加单一聚合校验入口、用户原话/决策的语义权限、范围关系以及 READY 写入边界。`不能只有 A` 不再被转换为“不做 A”；任一子校验失败都会阻止 READY；开始确认必须写明项目根目录、允许写入位置、受保护样例、计划路径和回退点。该版本当前仅为本地候选，尚未发布到 GitHub。

`0.8.0-rc8` 在 rc7 成熟需求收集流程上增加两组控制。新手模式只改变表达方式：首轮仍使用未回答的 Q1-Q4 单一入口，资料不可定位时在同一轮询问位置，证据成熟后只集中询问会改变下一步路线的决策；不会增加资料菜单、项目级别选择或重复开工卡。已有项目采用“当前基线 + 本次变更包 + 稍后清单”，READY 只能显示已通过聚合校验的行为，实施验证通过后才合并到基线；验收同时检查本次功能和既有能力未回退。本地配套的独立 `allred-project-memory` Skill 也已同步支持变更状态、验证后合并及失败不入基线，但不包含在本仓库的标准 Skill 安装包中。本版本作为 GitHub 预发布候选，不替代 `0.7.1` 稳定版。

`0.8.0-rc9` 删除独立的新手项目路由，只保留可随时切换的新手表达层。`allred新项目` 与 `allred新手项目` 使用同一套需求收集、证据、决策、READY、执行和验收流程；区别只在措辞、解释深度和回答便利性。该候选同时禁止授权前向项目目录写入分析产物或缓存，提供系统临时证据目录脚本，并强化聚合校验与通用证据层级：资料观察、中间产物、组件行为、端到端流程和目标环境验收不能相互替代，元数据、局部样本或单项检查不能被说成已经验证完整集成、普遍适用或最终可用。新增同题表达切换、真实工作区写入探针，以及 PDF、表格和日志等跨场景回归；这些案例只用于验证通用流程，不会形成领域专用分支。本版本尚未发布到 GitHub。

`0.8.0-rc10` 保留唯一普通用户入口，把多人共享协作、外部来源和公司办公交付拆成证据确认后才加载的条件能力层。外部来源进一步区分一次性查询与持续监测；普通本地项目、培训和调试不再读取无关的权限、审计、来源监测或办公部署规则。Skill 改进、场景评测和发布审查由显式维护 Skill `allred-project-lab` 接管；静态检查改为结构化 invariant 所有权与覆盖校验，语义表现由软件、培训、外部数据、多人协作、调试和长期任务行为测试负责。

`0.8.0-rc11` 在 rc10 基线上收紧通用性：外部证据不再强制所有项目回答固定的主题、类别、语言和地区四项，而是从当前项目契约中选择真正影响可用性或验收的语义维度；培训仍做完整内部覆盖检查，但只向用户显示非空且相关的内容，不再生成空的暂缓/排除占位章节。新增运行时通用性门禁，防止回归测试中的产品名称、场景字段或固定展示方式进入主流程；普通用户触发词、成熟需求收集、阶段校验和条件能力层保持不变。

`0.8.0-rc12` 针对“已登记问题全部关闭，但仍有重要需求领域未被发现”的过早 READY 问题，新增需求覆盖完整性门禁。它不限制问题数量，也不增加固定用户问卷；Codex 在资料分析后从工作流程、信息关系、生命周期/异常、实际规模、交付影响和验收六个内部视角复核当前项目，并把证据限制、已确认决策和最终范围连接起来。复杂项目发现新缺口时可以继续多轮集中询问；简单且证据完整的项目仍直接进入一次完整开始确认。本版本已作为 GitHub 预发布候选，不替代 `0.7.1` 稳定版。

`0.8.0-rc13` 修复自动化动态评测发现的阶段边界缺陷：用户说资料齐全但没有给出可读取位置时，Codex 先在当前工作区做一次有限搜索；没有路径、附件或可信命中就保持 INTAKE，只询问准确位置，不再通过读取 EVIDENCE 规则假装进入证据阶段。维护端将 FailFast 后未执行案例标记为 `NotRun`，并增加多试验一致性评测：独立保留每次 transcript 和 review，自动复测非通过案例，区分稳定通过、稳定失败、模型方差和基础设施不确定，避免由单次随机结果决定发布结论。

`0.8.0-rc14` 针对“集中决策变成一次性展开全部未决项”的体验问题增加单次决策包认知负荷门禁。完整决策前沿仍由内部状态保存，但普通可见问题包一次最多展示 4 个当前最有决定力的 `Q/D`，其余项目只显示待后续确认的数量，不展开选项，也不视为批准、暂缓或排除。问题深度和总轮次不设固定上限；用户回答后继续下一组，避免单次回复像规格书一样过长。
...
普通安装只安装 `allred-project-standard`。只有维护和发布本 Skill 的电脑才需要显式安装 `allred-project-lab`：

```powershell
$labTarget = "$env:USERPROFILE\.codex\skills\allred-project-lab"
New-Item -ItemType Directory -Force -Path $labTarget | Out-Null
Copy-Item .\maintainer\allred-project-lab\* $labTarget -Recurse -Force
```

`allred-project-lab` 设置为 `allow_implicit_invocation: false`，必须使用 `$allred-project-lab` 显式调用。
...
也可以使用简短别名 `allred新手`，在当前项目上切换为更容易理解的表达。单独出现“新手员工”等普通描述不会触发新手表达层。
...
维护 Allred Skill 本身时，显式使用维护入口（普通项目不使用）：

```text
$allred-project-lab
审查 allred-project-standard 的架构、行为回归和发布候选。
```
...
新手表达层只改变沟通方式，不降低开发标准，也不代表项目一定简单。它不是独立工作流：新手和标准表达共享相同的项目阶段、需求、范围、授权和验收。

它也不会因为用户选择了新手表达，就提前决定“先做一个小版本”。本轮是一次完成、先验证关键风险还是先补资料，要等资料、用户初版想法、来源风险和交付环境明确后再判断。
...
- 给出推荐默认值，回复 `1` 可全部确认
- 只有用户或资料明确说明公司办公电脑约束时，才加载对应交付建议；不会因新手表达擅自决定平台
...
pwsh -NoProfile -File .\allred-project-standard\scripts\run_entry_guard_eval.ps1 -UseUserConfig -DisablePlugins
```
...
稳定版本应使用与 `VERSION` 一致的 Git tag；当前稳定 tag 为 `v0.7.1`，最新 GitHub 预发布 tag 为 `v0.8.0-rc13`。本地 `rc14` 正在验证单次决策包长度和多轮前沿保留，验证完成后再决定是否发布新候选。
