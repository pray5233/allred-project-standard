...
当前本地候选版本：`0.8.0-rc12`。远端是否已发布以 GitHub Releases 和安装目录中的 `VERSION` 为准；安装回执用于确认实验机实际加载的提交与哈希。
...
`0.8.0-rc12` 针对“已登记问题全部关闭，但仍有重要需求领域未被发现”的过早 READY 问题，新增需求覆盖完整性门禁。它不限制问题数量，也不增加固定用户问卷；Codex 在资料分析后从工作流程、信息关系、生命周期/异常、实际规模、交付影响和验收六个内部视角复核当前项目，并把证据限制、已确认决策和最终范围连接起来。复杂项目发现新缺口时可以继续多轮集中询问；简单且证据完整的项目仍直接进入一次完整开始确认。当前版本为本地候选，尚未发布到 GitHub。
...
稳定版本应使用与 `VERSION` 一致的 Git tag；当前稳定 tag 为 `v0.7.1`，最新已发布 GitHub 预发布 tag 为 `v0.8.0-rc11`。本地 `rc12` 尚未发布，完成自动化和本机候选验证后再申请实验机验收。
