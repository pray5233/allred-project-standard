...
$stageGuards = @{
  ...
  evidence = "## Active Stage Guard`n`nCurrent stage: EVIDENCE. Output-filter hard stop: delete absent/unknown/untested unselected domains; saying 'not in scope' is leakage. GuardsOnly is never the first or only route load. Artifact-location hard stop: a label is not a target; no target means ask only for its location/sample. Before render/extract disclose unchanged originals/project plus disposable evidence only in isolated temp. Post-result exact call: get_route_context.ps1 -Route <route> -Stage evidence -Interaction <standard|beginner> -GuardsOnly; GuardsOnly is a switch, never an Interaction value. Then reload normal evidence context. Report observations, limits, and write boundary before another action. Preserve every named active-contract capability and evidence gap at facet level. Evidence-quantifier lint: preserve sample/subgroup/count/uncertainty; an aggregate does not prove each subgroup. Temp outputs are disposable evidence, not project artifacts. Unless blocked, continue authorized read-only work. Coverage-review before READY; gaps return to DECISION. Trusted DECISION/READY event: call get_route_context.ps1 -Route new-standard -Stage decision -ValidatedEventId <ID>. Script search is not a stage transition. No mutation."
