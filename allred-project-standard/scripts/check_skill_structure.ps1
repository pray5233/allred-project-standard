...
Require-Contains $evidenceGuardOnly 'never the first or only route load' 'evidence route entry requires full context'
...
Require-Contains $evidenceGuardOnly "Output-filter hard stop: delete absent/unknown/untested unselected domains; saying 'not in scope' is leakage" 'post-event conditional-overlay isolation'
Require-Contains $evidenceGuardOnly 'Preserve every named active-contract capability and evidence gap at facet level' 'named negative evidence coverage'
