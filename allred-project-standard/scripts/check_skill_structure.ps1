...
Require-Contains $evidenceGuardOnly 'never the first or only route load for a review' 'evidence route entry requires full context'
Require-Contains $evidenceGuardOnly 'reload the normal evidence route before interpreting new evidence' 'post-event synthesis requires full evidence context'
Require-Contains $evidenceGuardOnly 'Artifact-location hard stop:' 'missing inspection target evidence stop'
Require-Contains $evidenceGuardOnly 'a descriptive name or label is not a target' 'descriptive artifact label is not locatable evidence'
