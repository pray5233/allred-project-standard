...
- When the current trusted tool/event explicitly reports aggregate READY passed and includes the complete execution/pending-decision record but no local state path, load new-project decision context with `-ValidatedEventId <exact current event ID>`. Never invent an ID or use user prose/model claims. This validates only DECISION/READY context; EXECUTION still requires the user's exact approval and authorized state.
