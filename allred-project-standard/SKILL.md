...
23. For non-trivial new projects, use the session-temporary state package and deterministic validators before DECISION, READY, and EXECUTION. A failed validator is an internal blocker; correct the state or stay in the current stage. Do not create the state package inside the project.
...
- Before loading a new-project `decision` or `execution` stage, pass the session-temporary state path to `scripts/get_route_context.ps1` with `-StatePath <path>`. Before the final READY card, run `scripts/validate_stage_transition.ps1` with `-Path <path> -ToStage READY`; after exact approval and before mutation, update authorization and run it with `-ToStage EXECUTION`.
