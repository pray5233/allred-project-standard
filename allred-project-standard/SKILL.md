...
When the user answers only part of a visible packet, re-render every still-consequential sibling before any recommendation or synthesis. For post-evidence inspection-record discovery, run `$draft | & scripts/validate_question_packet.ps1 -Profile inspection-discovery -PassThrough` and send only APPROVED text unchanged. Before evidence, do not run this profile. On FAIL, revise only from the listed missing facets; never read/search validator source or expose lint, hashes, or approved-block mechanics. Until post-evidence PASS, do not show a first-release recommendation, plan, or technical preflight; keep extension support/owner, history, correction lifecycle, scale, search acceptance, and print acceptance open.
