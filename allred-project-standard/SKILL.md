...
- For an ordinary non-training/non-shared DECISION reply, expose only the next readable frontier slice and run `$draft | & scripts/validate_question_packet.ps1 -Profile decision-frontier -PassThrough`; send only the approved packet unchanged. Keep dependency/basis/deferral fields internal; do not print per-node why/basis/recommendation/reply paragraphs. Queue summary is count only: never name an unshown item or say it can be deferred, excluded, or approved.
...
## Partial Reply First Action

When the user answers only part of a visible packet, preserve every still-consequential sibling and show the next readable dependency-valid slice before any recommendation or synthesis; do not dump the whole remaining frontier into one response. For post-evidence inspection-record discovery, run `$draft | & scripts/validate_question_packet.ps1 -Profile inspection-discovery -PassThrough` and send only APPROVED text unchanged. Before evidence, do not run this profile. On FAIL, revise only from the listed missing facets; never read/search validator source or expose lint, hashes, or approved-block mechanics. Until post-evidence PASS, do not show a first-release recommendation, plan, or technical preflight; keep extension support/owner, history, correction lifecycle, scale, search acceptance, and print acceptance open.

## Exact Inspection First Action

For an exact read-only inspection, a descriptive label such as `this local PDF` is not a locatable target. After one bounded workspace search, if no path, attachment, or sample is found, the visible reply asks only for that target and does not announce inspection steps. Use only supported `Interaction` values: `standard` or `beginner`.
...
- Every visible INTAKE group states its own practical effect. The user/workflow/pain group explicitly changes actors, responsibility, workflow boundary, and which pain to prioritize; an opening summary never substitutes for that group-level effect.
...
- When current evidence makes unresolved user-facing choices concrete and their answers determine the next coherent preflight path, ask the highest-impact dependency-valid slice now in one concentrated decision packet. An ordinary packet contains at most four active `Q/D` nodes; remaining nodes stay open for the next group. Do not continue as though the user will volunteer those answers, do not call the packet final approval, and do not include answer-independent technical choices.
...
- When a factual `Q` explicitly allows uncertainty, `不知道/暂不确定/暂无数据` is a completed current-fact answer with value unknown. Do not count or ask it again, and do not invent a number; reopen it only when new evidence makes an exact value genuinely blocking.
- An unvalidated new-project DECISION call is not a user-facing blocker. Initial missing facts return to the complete INTAKE packet; a partial reply presents the next bounded sibling slice before technical preflight. Keep the queued count/status visible, but do not expand all options at once or hide them until after preflight.
...
- If the latest user reply answers only part of a visible packet and remaining siblings are already formulable, do not call EVIDENCE or begin read-only/technical preflight. Immediately show the next readable dependency-valid slice and preserve the rest as open.
...
- When evidence or a prior packet exposes extensibility/configuration owner, historical handling, correction/void lifecycle, operating scale, or measurable acceptance as open, each remains an independent sibling. Preserve every still-material facet across bounded packets; never invent numeric targets or let a recommendation settle ownership.
- Extensibility has two separate questions: whether future additions are supported and who configures them. Asking whether additions are expected or recommending that ordinary users should not act never answers configuration ownership.
...
- After debug evidence, choose one highest-value causal fork and one bounded experiment controlling one variable or comparison. State the observable result that supports versus weakens it; do not combine a comparison with added tracing as one experiment.
- For continuous monitoring evidence, load the `external-source` overlay with monitoring mode before synthesis. Preserve the confirmed schedule, retained history, failure visibility and missed-run recovery, source-change behavior, and maintenance owner in every handoff.
...
- Q/D IDs are stable references and optional shortcuts. Always accept natural prose. In an ordinary visible packet, keep each node compact and use one reply instruction for the whole group; internal dependency, basis, and deferral fields are not a display template.
...
- If the user names decision topics to revisit after evidence, keep every topic on the decision frontier. Once evidence makes their options and consequences explainable, order them into bounded concentrated packets; ask parents and highest-impact co-answerable nodes first. A dependent item waits only when the parent answer is genuinely required to formulate its options. Waiting for the next packet is not user deferral.
...
- Use `Q` for current facts and `D` for future meaning; never combine them. Track why, dependency, basis, option effects, and deferrability internally. Visible nodes show only the question, one impact, compact options, and an evidence-backed recommendation marker if available; use neutral trade-offs without evidence.
- When the user authorizes only discovery, analysis, design, or scope clarification, unresolved product `D` nodes remain deferrable unless required for the next safe read-only action. Name deferral and consequence only for a currently visible node; queued nodes are reported by count and remain unresolved. Never imply that the full frontier must be answered now.
...
- On question-packet lint failure, revise only from the reported reason and rerun the draft. Never inspect/search validator source, expose lint mechanics, or send a failed draft.
